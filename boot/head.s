 ...
 * 这些寄存器的名称和用途在x86汇编语言中是标准的，但在不同的汇编语法中（如AT&T语法和Intel语法）可能会稍有不同。通常，这些寄存器用于执行各种算术、逻辑、数据传输、堆栈操作和控制流操作。程序员可以根据需要使用这些寄存器来执行特定的任务。
 */

/* 在x86体系结构的32位保护模式下，段寄存器的寻址方式发生了变化，与实模式下的寻址方式有很大的不同。在保护模式下，段寄存器不再存储段的物理地址，
 * 而是存储一个称为 "段选择子"（Segment Selector）的值。这个段选择子用于索引全局描述符表（Global Descriptor Table，简称GDT）或局部描述符表（Local Descriptor Table，简称LDT）中的段描述符。
 * 每个段选择子由以下部分组成：
...
# 该返回指令的另一个作用是将堆栈周昂的main程序的地址弹出,并开始运行/init/main.c程序的
# 本程序到此就真正结束了
