 ...
 */
	// 下面代码开始以任务0的身份运行.
/*
 * 注意!! 对于任何其它的任务,'pause()'将意味我们必须等待收到一个信号才会返回就绪状态,但任务0(task0)是唯一的例外情况(参见schedule()),因为
...
static char * argv[] = { "-/bin/sh",NULL };      // argv[0]中的字符'-'是传递给shell程序sh的一个标志.通过识别该标志,sh 程序会作为登录 shell执行.其执行过程与在shell提示符下执行 sh 不太一样
...
{
	int pid,i;
	// 这是一个系统调用.用于读取硬盘参数包括分区表信息并加载虚拟盘(若存在的话)和安装根文件系统设备.该函数是用25行上的宏定义的,对应函数是
	// sys_setup(),在kernel/blk_drv/hd.c 的 71行
	...
	if (!(pid=fork())) {
		// 在C语言中，close(0); 是用于关闭文件描述符 0 的函数调用。文件描述符 0 通常是标准输入文件描述符，它代表标准输入流（stdin）。
		// 这个函数调用的作用是关闭标准输入流，这意味着后续的输入操作将无法从标准输入流中读取数据。通常情况下，不建议关闭标准输入流，
		// 因为它是程序与用户交互的主要方式。如果关闭了标准输入流，程序将无法从终端或其他标准输入设备中接受输入。
		// 如果你在程序中看到 close(0); 这样的代码，通常是因为程序需要关闭不再需要的文件描述符。如果确实需要关闭文件描述符，应该小心确保
		// 它们不再在后续的代码中使用，以免引发意外错误。一般情况下，关闭标准输入流并不是一个常见的操作，除非你有明确的理由要这样做。
		close(0);
		// /etc/rc 是一个与启动和运行Linux系统有关的重要目录和文件。它包含了一些与系统初始化和启动过程相关的脚本和配置文件。
		// 这个目录下的文件和子目录的具体名称和功能可能因Linux发行版而异
		if (open("/etc/rc",O_RDONLY,0))    // 以只读方式打开/etc/rc文件
			_exit(1);                      // 如果打开文件失败,则退出

		// execve 函数是一个在Unix和Linux系统上的系统调用，用于执行一个新的程序。它的主要作用是加载一个新的可执行文件到当前进程的地址空间，并开始执行该可执行文件的代码。
		// execve 函数通常用于创建新的进程，因为它能够在当前进程的上下文中启动一个新的程序。
		// 下面是 execve 函数的一些重要特性和作用：
		// 加载新程序：execve 函数将一个新的可执行文件加载到当前进程的地址空间中，覆盖了当前进程的代码和数据。这个新程序将取代当前进程的执行。
		// 参数传递：execve 函数允许传递参数给新程序。这些参数通常是命令行参数，它们可以在新程序中通过 main 函数的参数列表访问。
		// 环境变量：execve 函数也允许传递环境变量给新程序。这些环境变量包含了关于进程运行环境的信息，例如路径、系统配置等。
		// 文件路径：execve 函数需要指定要执行的可执行文件的路径。通常，该路径可以是相对路径或绝对路径。
		// 返回值：如果 execve 成功执行，它将不会返回，因为当前进程已经被新程序替代。如果 execve 失败，它将返回一个负数，通常是 - 1，表示出现了错误。
		// 可以使用 errno 来获取失败的具体原因。
		// 错误处理：execve 可能失败，例如当指定的可执行文件不存在或权限不足时。在这种情况下，应该根据错误码来进行适当的错误处理。
		// execve 是一个非常强大的系统调用，它允许在一个进程内启动新的程序，因此它在创建新进程、执行外部命令以及在守护进程中执行不同程序时非常有用。
		// 通常，它与 fork 或 clone 等系统调用一起使用，以在新进程中执行新程序。
		execve("/bin/sh",argv_rc,envp_rc); // 将进程自身替换成 /bin/sh 程序(即 shell 程序) 
	...
	// 如果执行到这里,说明刚创建的子进程的执行已停止或终止了.下面循环中首先在创建一个子进程.如果出错,则显示"初始化程序创建子进程失败"信息并继续执行.对于
